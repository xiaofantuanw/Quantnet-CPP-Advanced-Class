...
public:
	...
	//Distance functions
	double Distance( std::shared_ptr<DistanceStrategy> ds) const; // Calculate the distance to the origin (0, 0).(const)
	double Distance(const Point& p, std::shared_ptr<DistanceStrategy> ds) const; // Calculate the distance between two points.(const)
	//Operators
